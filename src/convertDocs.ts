...
const { a: docs, A: mainTitles } = require("../openai-docs-reference.js") as {
  a: DocItem[];
  A: any[];
};
...
const OUTPUT_DIR = path.resolve(
  path.join(__dirname, "../openai-docs-api-reference"),
);
...
function convertDocsToMarkdown(docs: DocItem[]): void {
  ...
  for (const doc of docs) {
    ...
    if (doc.type === "endpoint") {
      ...
      console.warn(
        `Potentially unhandled document type or format: ${doc.type} - ${doc.title}`,
      );
    }
    ...
    if (!generatedSubstantialContent) {
      ...
      //    that produce no useful output beyond title.
        ...
        !isContainerDoc &&
        markdownContent.trim() === `# ${doc.title}`.trim()
      ) {
        skipFileCreation = true;
      }
...
function convertEndpointToMarkdown(doc: EndpointDoc): string {
  ...
  if (
    doc.definition &&
    doc.definition.parameters &&
    doc.definition.parameters.length > 0
  ) {
    markdown += `## Parameters\n\n`;

    const pathParams = doc.definition.parameters.filter((p) => p.in === "path");
    const queryParams = doc.definition.parameters.filter(
      (p) => p.in === "query",
    );
    const headerParams = doc.definition.parameters.filter(
      (p) => p.in === "header",
    );
    const cookieParams = doc.definition.parameters.filter(
      (p) => p.in === "cookie",
    );
  ...
  if (doc.definition && doc.definition.requestBody) {
    ...
    if (content) {
      for (const mediaType in content) {
        if (
          Object.prototype.hasOwnProperty.call(content, mediaType) &&
    ...
    for (const [code, response] of Object.entries(doc.definition.responses)) {
      ...
      if (response.content) {
        for (const mediaType in response.content) {
          if (
            Object.prototype.hasOwnProperty.call(response.content, mediaType) &&
  ...
  if (examples) {
    ...
    if (examples.response) {
      ...
      markdown += `\`\`\`json\n${
        typeof examples.response === "string"
          ? examples.response
...
function convertDefinitionToMarkdown(doc: DefinitionDoc): string {
  ...
  if (doc.definition && doc.definition.properties) {
    markdown += `## Properties\n\n`;
    markdown += renderPropertiesTable(
      doc.definition.properties,
      doc.definition.required || [],
    );
    ...
    for (const [name, prop] of Object.entries(doc.definition.properties)) {
      const required =
        doc.definition.required && doc.definition.required.includes(name)
          ? " (required)"
          : "";
      ...
      if (prop.enum) {
        markdown += `**Allowed values**: ${prop.enum
          .map((v) => `\`${v}\``)
          .join(", ")}\n\n`;
      }
...
function renderPropertiesTable(
  ...
  for (const [name, prop] of Object.entries(properties)) {
    ...
    if (prop.properties && Object.keys(prop.properties).length > 0) {
      const nestedTable = renderPropertiesTable(
        prop.properties,
        prop.required || [],
        indent + 1,
      );
      ...
      markdown += `| ${indentation}  ↳ (additional properties) | ${getTypeString(
        prop.additionalProperties,
      )} | - | - | - | Additional properties of this object |\n`;
    }
  }
  ...
  for (const [name, prop] of Object.entries(properties)) {
    if (prop.type === "array" && prop.items) {
      if (
        ...
        const itemDescription = prop.items.description
          ? ` - ${cleanDescription(prop.items.description)}`
          : "";
        const itemEnum = prop.items.enum
          ? `. Allowed values: ${prop.items.enum
              .map((v) => `\`${v}\``)
              .join(", ")}`
          : "";
        const itemDefault =
          prop.items.default !== undefined && prop.items.default !== null
            ? `. Default: \`${
                typeof prop.items.default === "object"
                  ? JSON.stringify(prop.items.default)
                  : prop.items.default
              }\``
            : "";
...
function renderSchema(schema: SchemaObject, level: number = 0): string {
  ...
  const example =
    schema.example || (schema["x-oaiMeta"] && schema["x-oaiMeta"].example);
  if (example) {
    markdown += `**Example:**\n\n\`\`\`json\n${
      typeof example === "string" ? example : JSON.stringify(example, null, 2)
    }\n\`\`\`\n\n`;
        ...
        segments: {
          ...
          items: {
            ...
            properties: {
              ...
              tokens: {
                type: "array",
                description: "Token IDs for the segment.",
                items: { type: "integer" },
              },
              temperature: {
                type: "number",
        ...
        segments: [
          {
            ...
            tokens: [
              50364, 440, 7534, 390, 257, 3743, 4008, 322, 257, 2368, 4266, 786,
              13, 50530,
            ],
            temperature: 0.0,
 ...
 */
function generateSpecialObjectMarkdown(
  objectStructure: SpecialObjectDoc,
): string {
  let markdown = "";
